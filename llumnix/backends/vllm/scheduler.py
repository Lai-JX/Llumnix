...
class SchedulerLlumnix(Scheduler):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.instance_migrating_out_requests_last_stage: Dict[str, Dict[str, SequenceGroupLlumnix]] = defaultdict(dict)
    ...
    def add_migrating_out_request_last_stage(self, dst_instance_id: str, backend_request: SequenceGroupLlumnix) -> None:
        self.instance_migrating_out_requests_last_stage[dst_instance_id][backend_request.request_id] = backend_request

    def pop_migrating_out_request_last_stage(self, dst_instance_id: str, request_id: str) -> None:
        assert dst_instance_id in self.instance_migrating_out_requests_last_stage and \
            request_id in self.instance_migrating_out_requests_last_stage[dst_instance_id], \
            "the request id of migrating out request in last stage should exist in migrating out request last stage"
        self.instance_migrating_out_requests_last_stage[dst_instance_id].pop(request_id)

    def free_migrating_out_requests_last_stage(self, dst_instance_id: str) -> List[SequenceGroupLlumnix]:
        migrating_out_requests_last_stage = list(self.instance_migrating_out_requests_last_stage[dst_instance_id].values())
