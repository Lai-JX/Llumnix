...
BASE_DIR='/workspace/llm-serve/Llumnix/logs/l40-pdd--'$TOTAL_INSTANCES/$MODEL/$DISTRIBUTION/$MIGRATION_BACKEND
mkdir -p $BASE_DIR
...
Llumnix_benchmark() {
    ...
    # filename=$BASE_DIR/benchmark_$((count))\_tp$TP\_$REQ_NUM\_qps_$QPS\_prompt_len_$prompt_len\_response_len_$response_len\_latency_info.json
    # if [ -e $filename ]; then
...
Llumnix_benchmark_pdd() {
    ...
    # filename=$BASE_DIR/benchmark_pdd_tp$TP\_$REQ_NUM\_qps_$QPS\_prompt_len_$prompt_len\_response_len_$response_len\_$((prefill_count))\_$decode_count\_latency_info.json
    # if [ -e $filename ]; then
...
for sm_clock in "${sm_clocks[@]}"; do
    for mem_clock in "${mem_clocks[@]}"; do
        ...
        # for response_len in 128 256 512 1024; do
        #     for prompt_len in 32 64 128 256 512 1024; do
        for response_len in 128; do
            for prompt_len in 256; do
